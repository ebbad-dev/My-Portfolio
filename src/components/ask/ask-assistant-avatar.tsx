...
function AssistantScene({
  ...
  useFrame(({ clock }) => {
    ...
    if (headRef.current) {
      ...
      const scale = clicked ? 1.075 : thinking ? 1.035 + Math.sin(time * 5) * 0.018 : attentive ? 1.035 : 1.02;
    ...
    if (leftEyeRef.current) leftEyeRef.current.scale.set(attentive || clicked ? 1.08 : 1, blink * (clicked ? 1.16 : success ? 1.1 : 1), 0.32);
    if (rightEyeRef.current) rightEyeRef.current.scale.set(attentive || clicked ? 1.08 : 1, blink * (clicked ? 1.16 : success ? 1.1 : 1), 0.32);
    if (leftPupilRef.current) {
      leftPupilRef.current.position.set(-0.31 + pupilX, 0.16 + pupilY, 0.61);
      leftPupilRef.current.scale.setScalar(clicked ? 1.15 : 1);
    }
    if (rightPupilRef.current) {
      rightPupilRef.current.position.set(0.31 + pupilX, 0.16 + pupilY, 0.61);
      rightPupilRef.current.scale.setScalar(clicked ? 1.15 : 1);
    }
    if (smileRef.current) {
      smileRef.current.scale.set(clicked || success ? 1.28 : attentive ? 1.12 : 1.06, clicked ? 0.22 : success ? 0.2 : 0.18, 0.1);
    }
  ...
  return (
    <>
      ...
      <pointLight position={[2.4, 2.4, 2.8]} intensity={2.05} color="#67e8f9" />
      <pointLight position={[-2.2, -1.8, 2.2]} intensity={1.35} color="#8b5cf6" />
      <pointLight position={[0, -1.2, 2.4]} intensity={0.85} color="#22d3ee" />
      <group ref={haloRef}>
      ...
      <group ref={headRef} position={[0, compact ? 0 : 0.16, 0]}>
        <mesh scale={[1.2, 0.98, 0.72]} position={[0, 0.13, 0]}>
          <sphereGeometry args={[0.82, 48, 32]} />
          <meshPhysicalMaterial color="#e6fbff" transparent opacity={0.42} roughness={0.12} metalness={0.18} clearcoat={1} transmission={0.32} thickness={0.5} />
        </mesh>
        <mesh position={[-0.88, 0.13, 0.04]} scale={[0.28, 0.44, 0.22]}>
          <sphereGeometry args={[0.72, 28, 18]} />
          <meshStandardMaterial color="#1d4ed8" emissive="#1d4ed8" emissiveIntensity={0.35} roughness={0.22} metalness={0.28} />
        </mesh>
        <mesh position={[0.88, 0.13, 0.04]} scale={[0.28, 0.44, 0.22]}>
          <sphereGeometry args={[0.72, 28, 18]} />
          <meshStandardMaterial color="#1d4ed8" emissive="#1d4ed8" emissiveIntensity={0.35} roughness={0.22} metalness={0.28} />
        </mesh>
        <mesh position={[-0.88, 0.13, 0.32]} scale={[0.42, 0.62, 0.1]}>
          <sphereGeometry args={[0.32, 24, 14]} />
          <meshStandardMaterial color="#60a5fa" emissive="#3b82f6" emissiveIntensity={0.55} roughness={0.2} />
        </mesh>
        <mesh position={[0.88, 0.13, 0.32]} scale={[0.42, 0.62, 0.1]}>
          <sphereGeometry args={[0.32, 24, 14]} />
          <meshStandardMaterial color="#60a5fa" emissive="#3b82f6" emissiveIntensity={0.55} roughness={0.2} />
        </mesh>
        <mesh position={[0, 1.08, 0.02]} rotation={[0, 0, 0.25]}>
        ...
        <mesh position={[0.13, 1.26, 0.08]}>
          <sphereGeometry args={[0.065, 18, 12]} />
          <meshStandardMaterial color="#a5f3fc" emissive="#22d3ee" emissiveIntensity={1.6} />
        </mesh>
        <mesh scale={[1.02, 0.6, 0.22]} position={[0, 0.08, 0.5]}>
          <sphereGeometry args={[0.64, 48, 24]} />
          <meshStandardMaterial color="#03111f" roughness={0.18} metalness={0.24} emissive="#061827" emissiveIntensity={0.82} />
        </mesh>
        <mesh position={[0, 0.33, 0.526]} scale={[1.08, 0.1, 0.04]}>
          <boxGeometry args={[1, 1, 1]} />
          <meshBasicMaterial color="#67e8f9" transparent opacity={0.32} />
        </mesh>
        <mesh ref={leftEyeRef} position={[-0.31, 0.17, 0.595]} scale={[1.08, 1.06, 0.32]}>
        ...
        <mesh ref={leftPupilRef} position={[-0.31, 0.16, 0.61]} scale={[1.08, 1.08, 0.3]}>
          <sphereGeometry args={[0.052, 18, 12]} />
          <meshBasicMaterial color="#052b32" />
        </mesh>
        <mesh ref={rightEyeRef} position={[0.31, 0.17, 0.595]} scale={[1.08, 1.06, 0.32]}>
          <sphereGeometry args={[0.105, 28, 18]} />
          <meshStandardMaterial color="#14f1dc" emissive="#14f1dc" emissiveIntensity={status === "thinking" ? 2.8 : status === "clicked" ? 2.45 : 1.75} />
        </mesh>
        <mesh ref={rightPupilRef} position={[0.31, 0.16, 0.61]} scale={[1.08, 1.08, 0.3]}>
          <sphereGeometry args={[0.052, 18, 12]} />
          <meshBasicMaterial color="#052b32" />
        </mesh>
        <mesh ref={smileRef} position={[0, -0.13, 0.612]} rotation={[0, 0, Math.PI]} scale={[1.06, 0.18, 0.1]}>
          <torusGeometry args={[0.24, 0.02, 10, 56, Math.PI]} />
          <meshBasicMaterial color={status === "success" || status === "clicked" ? "#34d399" : "#22d3ee"} transparent opacity={0.94} />
        </mesh>
        <mesh position={[0, -0.78, 0.02]} scale={[0.84, 0.44, 0.5]}>
          <sphereGeometry args={[0.62, 40, 20]} />
          <meshPhysicalMaterial color="#d7fbff" transparent opacity={0.32} roughness={0.16} metalness={0.12} clearcoat={1} transmission={0.25} />
        </mesh>
        <mesh position={[0, -0.78, 0.45]} scale={[0.88, 0.26, 0.08]}>
          <boxGeometry args={[1, 1, 1]} />
  ...
  return (
    <button
      ...
      className={cn(
        ...
        compact ? "h-12 w-12 rounded-2xl" : "h-[clamp(12rem,24vw,15rem)] w-full",
    ...
    >
      ...
      {!compact ? (
        <div className="pointer-events-none absolute left-3 top-3 z-10 flex flex-wrap gap-1.5">
          {["Guide Online", "Projects Indexed", "Resume Ready"].map((label) => (
      ...
      <Canvas
        ...
        camera={{ position: [0, 0, compact ? 4.55 : 3.65], fov: compact ? 40 : 36 }}
        dpr={[1, 1.45]}
        gl={{ antialias: true, alpha: true, powerPreference: "high-performance" }}
      >
        <AssistantScene pointer={pointer} status={expression} reducedMotion={reducedMotion} compact={compact} />
      </Canvas>
      <div className="pointer-events-none absolute inset-x-5 bottom-2 hidden items-center justify-center gap-2 rounded-full border border-white/10 bg-slate-950/42 px-3 py-1 text-[9px] font-semibold text-slate-400 backdrop-blur sm:flex">
